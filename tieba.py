...
def get_content(url):
    ...
    for li in liTags:
        ...
        content['responseNum'] = li.find('span', attrs={'class': 'threadlist_rep_num'}).text.strip()

        content['creatTime'] = li.find('span', attrs={"class":'is_show_create_time'}).text.strip()
...
def writeTxt(content):
    ...
    f = open("newData.txt", 'a+', encoding='utf-8')
    ...
    for c in content:
        ...
        creatTime.append(c['creatTime'])
        responseNum.append(c['responseNum'])
    '''
    data['title'] = title
    data['link'] = link
    data['author'] = author
    data['creatTime'] = creatTime
    data['responseNum'] = responseNum
    
    data.to_csv('tieba.csv')'''
    tiezi = []
    tiezi.append(title)
    tiezi.append(link)
    tiezi.append(author)
    tiezi.append(creatTime)
    tiezi.append(responseNum)
    return tiezi


url = "https://tieba.baidu.com/f?kw=南昌大学&ie=utf-8"
page = 50
...
def main(url, page):
    ...
    for u in url_list:
        ...
        tiezi=writeTxt(content)

        title = title+tiezi[0]
        link = link+tiezi[1]
        author = author+tiezi[2]
        creatTime = creatTime+tiezi[3]
        responseNum = responseNum+tiezi[4]
